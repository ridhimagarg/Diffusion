...
def main():
    ...
    for ref_act in ref_acts:    
        ref_stats.append(evaluator.read_statistics(args.ref_batch, ref_act)[0])
    ...
    for sample_act in sample_acts:
        sample_stats.append(evaluator.read_statistics(args.sample_batch, sample_act)[0])
        sample_stats_spatial.append(evaluator.read_statistics(args.sample_batch, sample_act)[1])

    logger.log("Computing evaluations...")

    for i in range(len(sample_stats)):

        logger.log("Inception Score:", evaluator.compute_inception_score(sample_acts[i][0]))
        logger.log("FID:", sample_stats[i].frechet_distance(ref_stats[i]))
        logger.log("sFID:", sample_stats_spatial[i].frechet_distance(ref_stats_spatial[i]))
        prec, recall = evaluator.compute_prec_recall(ref_acts[i][0], sample_acts[i][0])
...
class Evaluator:
    ...
    # def create_class_wise_batches(labels, images):
    ...
    def read_activations(self, npz_path: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        with open_npz_array(npz_path, "arr_1") as reader:
            array_labels = reader.read_batches(self.batch_size)
            data_labels = np.empty((5000))
        ...
        with open_npz_array(npz_path, "arr_0") as reader:
            ...
            data_images = np.empty((5000,  *(next(iter(reader.read_batches(self.batch_size)))[0].shape)))
            array_images = reader.read_batches(self.batch_size)

            index  = 0
            for batch in array_images:
                data_images[index:index+batch.shape[0]] = batch
                index = index +batch.shape[0]
            # print(data_images.shape)
            # print("labels", data_labels)
            # print("images", data_images[10:20])
        ...
        final_images = {}
        print(data_images.shape[1:])
        for idx, class_ in enumerate(classes):
            final_images[class_] = np.empty((counts[idx], *(data_images.shape[1:])))

        labels_idx = counts.copy()
        for idx, label in enumerate(data_labels):
            # print("Label", label)
            # print("Counts", counts[int(label)])
            # # final_images[label] = np.append(final_images[label], data_images[idx])
            # print("Index", counts[int(label)] - labels_idx[int(label)])
            final_images[label][counts[int(label)] - labels_idx[int(label)]] = data_images[idx]
            labels_idx[int(label)] -= 1
        ...
        for class_, images in final_images.items():
            ...
            with open_npz_array(npz_path, "arr_0") as reader:
                activations.append(self.compute_activations(reader.read_batches(self.batch_size)))
        return activations
...
class StreamingNpzArrayReader(NpzArrayReader):
    ...
    def read_batch(self, batch_size: int) -> Optional[np.ndarray]:
        ...
        # print("Array", self.arr_f)
        # print("Loaded array", np.load(self.arr_f))
...
class MemoryNpzArrayReader(NpzArrayReader):
    ...
    def load(cls, path: str, arr_name: str):
        with open(path, "rb") as f:
            ...
            class_name = np.load(f)["arr_1"]
        return cls(arr), cls(class_name)
