...
def calculate_activation_statistics(arr_obj, model, batch_size=50, dims=1024,
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the inception model.
    """
    act = get_activations(arr_obj, model, batch_size, dims, device, num_workers)
...
def compute_statistics_of_path(path, model, batch_size, dims, device,
    ...
    if path.endswith('.npz'):
        ...
        arr_obj = np.load(path)
        if "mu" in list(arr_obj.keys()):
             m, s = arr_obj['mu'][:], arr_obj['sigma'][:]
        else:
            m, s = calculate_activation_statistics(arr_obj, model, batch_size,
...
def main():
    ...
    fid_value = calculate_fid_given_paths(args.path,
                                          args.batch_size,
                                          device,
                                          1024, ## choosing 1024 dimensions
                                          num_workers)
    
    logger.log("FID: ", fid_value)
    print('FID: ', fid_value)
